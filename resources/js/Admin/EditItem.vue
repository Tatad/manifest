<script  lang="ts" setup>
	...
	const itemResult = ref(props.itemData)
	const form = useForm({
	    description: itemResult.value.description,
	    item_number: itemResult.value.item,
	    type: itemResult.value.type,
	    pallet: itemResult.value.pallet,
    	image: [],
	    quantity: itemResult.value.quantity,
	    msrp: itemResult.value.msrp,
	    upc_code: itemResult.value.upc_code,
	    retail_price: itemResult.value.retail_price,
	    errors: []
	});
	...
	const image = ref('');
  	const imageThumb = ref(itemResult.value.images);
  	const MAX_WIDTH = 620;
	...
	const submit = () => {
		form.post(route('admin.editItem'), {
	        onSuccess: () => {
